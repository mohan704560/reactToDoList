...
const TaskListGrid = ({ task }) => {
  ...
  return (
    <>
      <div className="grid grid-flow-col grid-cols md:grid-cols-2 lg:grid-cols-3 gap-8 grid-flow-row">
        {/* to render the list of the all task in the dom */}
        {task.length > 0 &&
          task.map((task) => (
            <div
              className="flex items-center h-32 border-2 border-gray-300 px-4"
              key={task.id}
            >
              {/* complete or incomplete checkbox  */}
              <div
                className="w-4 h-4 me-4 appearance-none border-2 rounded-xs"
